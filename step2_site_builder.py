...
OUTPUT_DIR = "dist"
LIMIT_PAGES = 500  # SEO 策略：首批上线 500 页
...
<body class="bg-slate-50 min-h-screen font-sans text-slate-900">
    <nav class="bg-white border-b border-slate-200 py-4 shadow-sm">
        <div class="max-w-5xl mx-auto px-4 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2">
                ...
                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Michael V3.9 Secure</span>
            </div>
        </div>
    </nav>
            ...
            <div class="p-10">
                <div id="drop-zone" class="relative border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center transition-all cursor-pointer hover:border-indigo-400 hover:bg-slate-50 group">
                    ...
                    <div id="upload-ui">
                        ...
                        <p class="text-slate-400 mt-3 italic uppercase text-xs tracking-widest font-bold">Target Standard: {{laws}}</p>
                    </div>
                ...
                <div id="result-ui" class="hidden mt-10 border-t pt-10 animate-in">
                    <div class="bg-green-50 border border-green-100 p-6 rounded-3xl mb-8 flex items-center justify-between">
                        <div>
                            <h4 class="text-green-800 font-bold text-lg">Document Processed Successfully!</h4>
                            <p class="text-green-600">The basic {{action}} task is complete.</p>
                        </div>
                        <button onclick="alert('Starting free download...')" class="bg-green-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-green-700 transition-all shadow-md">
                            Free Download
                        </button>
                    </div>
                    ...
                    <div class="bg-indigo-50 border border-indigo-100 p-8 rounded-[2rem]">
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="flex-1">
                                <h4 class="text-indigo-900 font-black text-xl mb-2 italic uppercase tracking-tight">Compliance Alert: High Risk Detected</h4>
                                <p class="text-indigo-700 mb-6 leading-relaxed">
                                    Our expert system detected <b>3 potential legal compliance risks</b> for <b>{{profession}}s</b> in this document. 
                                    We recommend an immediate <b>Expert Audit Report</b> to ensure full adherence to <b>{{laws}}</b>.
                                </p>
    ...
    <div id="pay-modal" class="fixed inset-0 bg-slate-900/95 hidden flex items-center justify-center z-50 p-4 backdrop-blur-md">
        <div class="bg-white p-12 rounded-[3rem] max-w-md w-full text-center shadow-2xl animate-in">
            <h3 class="text-3xl font-black text-slate-900 mb-4 tracking-tighter italic uppercase">Unlock Report</h3>
            ...
            <div class="relative py-6">
                ...
                <div class="relative flex justify-center text-xs uppercase tracking-widest text-slate-300 font-bold bg-white px-4 italic">Michael Admin Sync</div>
            </div>
    ...
    <script>
        ...
        runToolBtn.onclick = () => {
            ...
            runToolBtn.innerHTML = '<span class="animate-pulse tracking-widest uppercase">Analyzing...</span>';
            setTimeout(() => {
        ...
        bypassBtn.onclick = async () => {
            bypassBtn.innerText = "Generating Professional PDF...";
            ...
            try {
                ...
                if (data.report) {
                    ...
                    doc.setFontSize(10);
                    doc.text(`Target: {{profession}} | Standard: {{state}} Laws`, 105, 30, {align: "center"});
                    ...
                    alert("Error: " + (data.error || "DeepSeek Connection Failed."));
                }
            } catch (e) {
        ...
        with open(INPUT_CSV, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                with open(os.path.join(OUTPUT_DIR, fname), "w", encoding="utf-8") as out:
                    out.write(content)
                count += 1
            print(f"✅ Michael! V3.9 Build Ready: {count} pages generated with Free Download + Paid Audit.")
