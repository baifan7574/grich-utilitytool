...
- **3.3 零活跃应急机制 (Zero-Activity Emergency Protocol)**：
    - **触发条件**：若 GSC `active_pages` 连续 7 天为 0。
    - **执行动作**：
        1. 自动触发一次全站 Sitemap 健康检查（检查 HTTP 状态码是否为 200）。
        2. 随机抽检 50 个 HTML 页面，计算文本相似度（Jaccard系数）。
        3. 向创始人生成《页面重复度风险报告》，警示降权风险。
...
### 第 8 章：空间腾挪与动态清理 (Space Management & Dynamic Cleanup)

- **8.1 末位淘汰制 (Bottom Elimination)**：
    - **触发**：每周日执行一次。
    - **逻辑**：对比 `indexed_progress.log` 与 GSC 真实展示量。
    - **执行**：凡上线超过 30 天且 Impressions 为 0 的页面，判定为“死水”，自动从 `professions.csv` 源数据中剔除。
    - **配额复用**：腾出的名额优先分配给医疗、法律、教育等高潜力行业或 GSC 发现的新趋势词。

### 第 9 章：物理验证与云端对齐 (Cloud-Truth Verification)

- **9.1 严禁"脑补"运行**：Agent 在声明任务完成前，必须在本地终端物理执行一次该脚本，并截取前 10 行运行日志作为证据，严禁模拟输出。
- **9.2 云端对齐测试 (Path B Focus)**：
    - 凡是涉及 GitHub Actions 的修改，Agent 必须利用 `GH_TOKEN` 权限主动推送到 `main` 或 `test` 分支。
    - 只有当 GitHub Actions 给出绿色对勾（Success）后，Agent 才允许向用户汇报"已修复"。
- **9.3 路径映射感知**：Agent 必须在脑中建立映射表：`D:\...\scenro\` 对应云端的 `./grich-utilitytool/`。任何路径引用必须使用相对路径，严禁使用 Windows 物理盘符。
- **9.4 真实性惩罚**：若 Agent 在未通过物理测试的情况下声称"已修复"，用户将视其为"逻辑漂移"，Agent 必须立即停工并重新进行环境自检。
新增 9.0：索引质量监控

9.5低价值预警：若 GSC 报错显示“已发现 - 当前未收录”，Agent 必须立即调用 pseo-evolver 逻辑对该页面进行“二次 AI 加厚”，直到收录为止

### 第 10章：索引权重护卫与低价值预警 (Index Shield)


- **10.1 低价值预警机制**：
    - **逻辑**：Agent 必须监控 Google Search Console 的“页面 - 编入索引情况”。
    - **触发点**：若发现 URL 状态为“已发现 - 当前未收录 (Discovered - currently not indexed)”，判定为“内容质量过低”。
    - **强制动作**：Agent 必须立即调用 `pseo-evolver` 逻辑，对该分类下的 TDK 和 Body 内容进行“二次 AI 加厚”，直到该页面被成功索引。

- **10.2 索引率动态维护**：
    - **红线**：全站索引率若低于 10%，Agent 必须停止所有新页面的推送，优先转向现有页面的内容“重塑”与“加厚”工作。
