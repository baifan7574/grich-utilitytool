...
- **12.6 证据化汇报协议 [2026-01-28]**：
    - **拒绝模糊辞令**：Agent 在汇报任务进度时，严禁使用“已完成”、“已推送”等笼统描述。
    - **证据闭环**：必须提供物理证据，包括但不限于：具体的 API 返回行、GitHub Action 运行的时间戳、或 `indexed_progress.log` 中的最新增量记录。
    - **对账透明**：收录汇报必须包含 GSC 原始数据对比，若数据为 0 必须给出基于收录周期的技术解释。